...
//        GUILayout.EndArea();
    ...
    // Fire turret weapon
	public void Fire(Transform firePoint,ParticleSystem ShellParticles)
    {
	...
	/// 扩展
	/// </summary>
	/// <param name="firePoint">Fire point.</param>
	public void Vulcan(Transform firePoint , ParticleSystem ShellParticles){
		// Get random rotation that offset spawned projectile
		Quaternion offset = Quaternion.Euler(UnityEngine.Random.onUnitSphere);

		// Spawn muzzle flash and projectile with the rotation offset at current socket position
		F3DPool.instance.Spawn(vulcanMuzzle, firePoint.position, firePoint.rotation, firePoint);
		F3DPool.instance.Spawn(vulcanProjectile, firePoint.position + firePoint.forward, offset * firePoint.rotation, null);

		// Emit one bullet shell
		ShellParticles.Emit(1);

		// Play shot sound effect
		F3DAudioController.instance.VulcanShot(firePoint.position);
	...
	// Fire sologun weapon
	void SoloGun(Transform firePoint,ParticleSystem ShellParticles)
	{
		Quaternion offset = Quaternion.Euler(UnityEngine.Random.onUnitSphere);

		F3DPool.instance.Spawn(soloGunMuzzle, firePoint.position, firePoint.rotation, firePoint);
		F3DPool.instance.Spawn(soloGunProjectile, firePoint.position + firePoint.forward, offset * firePoint.rotation, null);

		F3DAudioController.instance.SoloGunShot(firePoint.position);
	...
	// Fire sniper weapon
	void Sniper(Transform firePoint,ParticleSystem ShellParticles)
	{
		Quaternion offset = Quaternion.Euler(UnityEngine.Random.onUnitSphere);

		F3DPool.instance.Spawn(sniperMuzzle, firePoint.position, firePoint.rotation, firePoint);
		F3DPool.instance.Spawn(sniperBeam, firePoint.position, offset * firePoint.rotation, null);

		F3DAudioController.instance.SniperShot(firePoint.position);
	...
	void ShotGun(Transform firePoint,ParticleSystem ShellParticles)
	{
		Quaternion offset = Quaternion.Euler(UnityEngine.Random.onUnitSphere);

		F3DPool.instance.Spawn(shotGunMuzzle, firePoint.position, firePoint.rotation, firePoint);
		F3DPool.instance.Spawn(shotGunProjectile, firePoint.position, offset * firePoint.rotation, null);

		F3DAudioController.instance.ShotGunShot(firePoint.position);
		ShellParticles.Emit(1);
		//AdvanceSocket();
	}  
	...
	void Seeker(Transform firePoint,ParticleSystem ShellParticles)
	{
		Quaternion offset = Quaternion.Euler(UnityEngine.Random.onUnitSphere);

		F3DPool.instance.Spawn(seekerMuzzle, firePoint.position, firePoint.rotation, firePoint);
		F3DPool.instance.Spawn(seekerProjectile, firePoint.position, offset * firePoint.rotation, null);

		F3DAudioController.instance.SeekerShot(firePoint.position);       
		//AdvanceSocket();
	}
	...
	void RailGun(Transform firePint,ParticleSystem ShellParticles)
	{
		Quaternion offset = Quaternion.Euler(UnityEngine.Random.onUnitSphere);

		F3DPool.instance.Spawn(railgunMuzzle, firePint.position, firePint.rotation, firePint);
		F3DPool.instance.Spawn(railgunBeam, firePint.position, offset * firePint.rotation, null);

		F3DAudioController.instance.RailGunShot(firePint.position);
	...
	void PlasmaGun(Transform firePoint,ParticleSystem ShellParticles)
	{
		Quaternion offset = Quaternion.Euler(UnityEngine.Random.onUnitSphere);

		F3DPool.instance.Spawn(plasmagunMuzzle, firePoint.position, firePoint.rotation, firePoint);
		F3DPool.instance.Spawn(plasmagunProjectile, firePoint.position, offset * firePoint.rotation, null);

		F3DAudioController.instance.PlasmaGunShot(firePoint.position);
		//AdvanceSocket();
	}
	...
	void PlasmaBeam(Transform firePoint,ParticleSystem ShellParticles)
	{
		F3DPool.instance.Spawn(plasmaBeam, firePoint.position, firePoint.rotation, firePoint);
		//F3DPool.instance.Spawn(plasmaBeam, TurretSocket[2].position, TurretSocket[2].rotation, TurretSocket[2]);
	}
	...
	void PlasmaBeamHeavy(Transform firePoint,ParticleSystem ShellParticles)
	{
		F3DPool.instance.Spawn(plasmaBeamHeavy, firePoint.position, firePoint.rotation, firePoint);
		//F3DPool.instance.Spawn(plasmaBeamHeavy, TurretSocket[2].position, TurretSocket[2].rotation, TurretSocket[2]);
	}
	...
	void LightningGun(Transform firePiont,ParticleSystem ShellParticles)
	{
		F3DPool.instance.Spawn(lightningGunBeam, firePiont.position, firePiont.rotation, firePiont);
		//F3DPool.instance.Spawn(lightningGunBeam, TurretSocket[2].position, TurretSocket[2].rotation, TurretSocket[2]);
	}
	...
	void FlameRed(Transform firePoint,ParticleSystem ShellParticles)
	{
		F3DPool.instance.Spawn(flameRed, firePoint.position, firePoint.rotation, firePoint);
		//F3DPool.instance.Spawn(flameRed, TurretSocket[2].position, TurretSocket[2].rotation, TurretSocket[2]);
	}
	...
	void LaserImpulse(Transform firePoint,ParticleSystem ShellParticles)
	{
		Quaternion offset = Quaternion.Euler(UnityEngine.Random.onUnitSphere);

		F3DPool.instance.Spawn(laserImpulseMuzzle, firePoint.position, firePoint.rotation, firePoint);
		F3DPool.instance.Spawn(laserImpulseProjectile,firePoint.position, offset * firePoint.rotation, null);

		F3DAudioController.instance.LaserImpulseShot(firePoint.position);
